...
class Psicologo(db.Model):
    ...
    ocr_verificado = db.Column(db.Boolean, default=False)
    biometrico_verificado = db.Column(db.Boolean, default=False)
    ...
    documento_acreditacion = db.Column(db.Text(length=4294967295), nullable=True) # LongText
    documento_dni = db.Column(db.Text(length=4294967295), nullable=True) # LongText
...
class Ticket(db.Model):
    ...
    estado = db.Column(db.String(20), default='abierto') # abierto, en_progreso, cerrado
    prioridad = db.Column(db.String(20), default='media') # baja, media, alta
    respuesta_admin = db.Column(db.Text, nullable=True)
    imagen_adjunta = db.Column(db.Text, nullable=True) # Base64 string o URL
    
    fecha_creacion = db.Column(db.DateTime, default=datetime.utcnow)
    fecha_actualizacion = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)

    # Relaciones
    paciente_rel = db.relationship('Paciente', backref=db.backref('tickets', lazy=True))
    psicologo_rel = db.relationship('Psicologo', backref=db.backref('tickets', lazy=True))
    mensajes_rel = db.relationship('TicketMensaje', backref='ticket_obj', cascade='all, delete-orphan', lazy=True)
    ...
    def to_dict(self):
        return {
            ...
            'imagen_adjunta': self.imagen_adjunta,
...
class TicketMensaje(db.Model):
    ...
    remitente = db.Column(db.String(100), nullable=False) # Nombre del usuario o 'Bot'
    tipo_emisor = db.Column(db.String(20), nullable=False) # 'paciente', 'psicologo', 'admin', 'bot'
    fecha_envio = db.Column(db.DateTime, default=datetime.utcnow)

    # Relación con Ticket (manejada desde Ticket con cascade)
